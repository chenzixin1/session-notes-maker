...
要启用无火山 Key 的本地转录 fallback，再安装可选 Whisper 依赖：
...
- `VOLCENGINE_API_KEY`：火山引擎新版控制台的 API Key；未填写时自动使用本地 Whisper

默认 Codex sub-agent 路径不需要 OpenRouter Key。`API_KEY`、`BASE_URL` 和 `MODEL` 只为兼容旧的 OpenRouter provider 保留，均为可选配置。

转录脚本默认使用 `--provider auto`：有效火山 Key 存在时使用录音文件极速版 HTTP 接口；没有 Key 时使用本地 OpenAI Whisper。火山路径把本地音频 Base64 编码后放入 `audio.data` 直接发送，无需对象存储、公网音频 URL，也无需 submit/query 轮询。旧版控制台鉴权可额外配置 `APP_KEY` 和 `ACCESS_KEY`。

本地 Whisper 默认使用 `small` 模型，可通过 `--whisper-model` 和 `--whisper-device` 调整。首次运行会下载所选模型。
...
1. **转录视频**
   ...
   - 脚本从视频抽取 16kHz 单声道 MP3；默认自动选择转录后端：有有效 Key 时通过 `audio.data` 直传火山引擎 ASR Flash，没有 Key 时运行本地 Whisper。
   - 可用 `--provider volcengine|whisper` 强制指定后端；本地路径支持 `--whisper-model` 与 `--whisper-device`。
...
- `2026-07-10`: 转录新增本地 Whisper fallback；未配置火山 Key 时自动使用本地 `small` 模型。
...
- 转录默认使用 `--provider auto`；火山 Key 缺失时应确认已安装 `requirements-whisper.txt`，不要把 Key 设为硬性前置条件。
